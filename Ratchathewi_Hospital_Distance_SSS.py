...
Generate Ratchathewi_Hospital_Distance_SSS.html — focused SSS-distance map for
เขตราชเทวี.

Behavior:
- Detect hospitals that accept สิทธิประกันสังคม (SSS) and compute nearest-SSS assignment
  for every community (global assignment among SSS hospitals).
- Show:
  - SSS hospitals located inside ราชเทวี (marker + popup).
  - Communities that are inside ราชเทวี OR are outside but assigned to an SSS hospital
    that is inside ราชเทวี (green markers).
  - Gray connection lines from those communities to their assigned SSS hospital.
  - SSS hospitals outside ราชเทวี that are linked from at least one community inside ราชเทวี
...
- Output: Ratchathewi_Hospital_Distance_SSS.html
"""
import json
...
OUT_HTML = "Ratchathewi_Hospital_Distance_SSS.html"
...
# visual choices
SSS_LINE_COLOR = "#9E9E9E"      # gray for connection lines
SSS_MARKER_COLOR = "#4CAF50"    # green for community markers
SSS_HOSPITAL_COLOR = "#c62828"  # red fallback for hospitals without icon
...
# ---------- Detect SSS hospitals ----------
sss_col = detect_sss_column(hospitals.columns)
if sss_col:
    hospitals['sss_accept'] = hospitals[sss_col].apply(truthy)
else:
    hospitals['sss_accept'] = False
    # fallback: search keyword in common text columns
    for col in hospitals.columns:
        if col.lower() in ('note','notes','type','remark','comment'):
            hospitals['sss_accept'] = hospitals['sss_accept'] | hospitals[col].astype(str).str.contains('ประกันสังคม|SSS|social', case=False, na=False)

sss_hospitals = hospitals[hospitals['sss_accept'] == True].copy()
print(f"Detected SSS column: {sss_col}; SSS hospitals found: {len(sss_hospitals)}")
...
# ---------- Nearest-SSS hospital for each community ----------
comm_assigned_sss = []
for c_idx, comm in communities.iterrows():
    try:
        clat = float(comm[LAT_COL]); clon = float(comm[LON_COL])
    except Exception:
        comm_assigned_sss.append((c_idx, None, None)); continue
    min_d = float('inf'); nearest = None
    for h_idx, hosp in sss_hospitals.iterrows():
        try:
    ...
    comm_assigned_sss.append((c_idx, nearest, min_d if min_d != float('inf') else None))

# compute SSS hospital weights (# communities assigned)
sss_hospitals['weight'] = 0
for c_idx, h_idx, d in comm_assigned_sss:
    if h_idx is not None and pd.notnull(h_idx):
        try:
            sss_hospitals.at[h_idx, 'weight'] += 1
        except Exception:
            pass
...
# ---------- SSS hospitals inside Ratchathewi (displayed) ----------
sss_hospitals_in = []
h_in_set = set()
for h_idx, hosp in sss_hospitals.iterrows():
    try:
    ...
    try:
        if target_shape.contains(pt):
            sss_hospitals_in.append((h_idx, hosp))
            h_in_set.add(h_idx)
    except Exception:
        continue
...
# - communities (outside) whose assigned nearest SSS hospital is inside ราชเทวี
comm_to_show = []
...
# ---------- Identify linked SSS hospitals outside ราชเทวี that are assigned-to by communities INSIDE ราชเทวี ----------
...
# ---------- SSS hospitals inside ราชเทวี ----------
sss_in_layer = FeatureGroup(name="SSS Hospitals (in ราชเทวี)", show=True, control=False).add_to(m)
HOSP_ICON_URI = try_file_name(HOSP_ICON_FN)
ICON_SIZE = (18,18); ICON_ANCHOR = (9,9)

for h_idx, hosp in sss_hospitals_in:
    try:
    ...
    try:
        folium.Marker(location=[latf, lonf],
                      ...
                      tooltip=title_esc).add_to(sss_in_layer)
    except Exception:
        folium.CircleMarker(location=[latf, lonf], radius=7, color=SSS_HOSPITAL_COLOR, fill=True, fill_color=SSS_HOSPITAL_COLOR,
                            popup=folium.Popup(popup_html, max_width=420), tooltip=title_esc).add_to(sss_in_layer)

# ---------- Linked SSS hospitals (outside ราชเทวี) - use Hospital.png icon if available ----------
...
for h_idx in sorted(linked_outside_hospitals_idx):
    ...
    """
    # prefer Hospital.png as icon for linked outside hospitals (tooltip shows only name)
    try:
        folium.Marker(location=[latf, lonf],
...
# ---------- Communities (green) and connections (gray) ----------
comm_layer = FeatureGroup(name="Communities (SSS connections)", show=True, control=False).add_to(m)
conn_layer = FeatureGroup(name="Filter Connections - สิทธิประกันสังคม", show=True, control=False).add_to(m)
HOUSE_ICON_URI = try_file_name(HOUSE_ICON_FN)

for c_idx, nearest_idx, dist_m in comm_assigned_sss:
    # show only communities selected (inside or assigned to in-district SSS)
    ...
    if nearest_idx is not None and pd.notnull(nearest_idx):
        try:
            hosp = sss_hospitals.loc[nearest_idx]
    ...
    <div style="background:#EAF3FF;color:#1A1A1A;font-family:'Bai Jamjuree',sans-serif;padding:10px;border-radius:8px;border:2px solid #6C7A89;max-width:320px;">
      ...
      <div style="margin-top:8px;font-size:14px;line-height:1.35;">
        <div><strong>โรงพยาบาลที่รับสิทธิประกันสังคมใกล้ที่สุด:</strong> {html.escape(str(hosp_name))}</div>
    ...
    folium.CircleMarker(location=[clat, clon],
                        radius=5.0,
                        color=SSS_MARKER_COLOR,
                        fill=True,
                        fill_color=SSS_MARKER_COLOR,
                        fill_opacity=0.95,
                        popup=folium.Popup(popup_html, max_width=360),
                        tooltip=str(comm_name)).add_to(comm_layer)

    # connection polyline (gray) to assigned SSS hospital if available
    if nearest_idx is not None and pd.notnull(nearest_idx):
        try:
            hosp = sss_hospitals.loc[nearest_idx]
            hlat = float(hosp[LAT_COL]); hlon = float(hosp[LON_COL])
            folium.PolyLine(locations=[[clat, clon], [hlat, hlon]],
                            color=SSS_LINE_COLOR, weight=1.6, opacity=0.85).add_to(conn_layer)
        except Exception:
            pass
...
print("SSS hospitals total:", len(sss_hospitals), "SSS hospitals in ราชเทวี:", len(sss_hospitals_in))
print("Linked outside SSS hospitals shown:", len(linked_outside_hospitals_idx))
print("Communities with SSS assignments shown:", len(comm_to_show))