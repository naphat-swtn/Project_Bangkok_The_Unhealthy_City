...
Fix: ensure district tooltips reliably open on hover.

What changed:
- Set GeoJSON path option 'interactive': True so polygons receive pointer events.
- Replaced the previous JS with a more robust binder that:
  - Ensures each district layer has a tooltip (binds one if missing).
  - Binds mouseover -> openTooltip and mouseout -> closeTooltip.
  - Keeps click-to-highlight behavior.
  - Calls bringToBack() several times to ensure markers remain clickable.
  - Logs diagnostic messages to console to help debugging.
...
for comm_idx, nearest_idx, dist_m in comm_assigned:
    ...
    <div style="background:#EAF3FF;color:#1A1A1A;font-family:'Bai Jamjuree',sans-serif;padding:10px;border-radius:8px;border:2px solid #6C7A89;max-width:320px;">
      <div style="font-weight:700;font-size:15px;">{html.escape(str(comm_name))}</div>
