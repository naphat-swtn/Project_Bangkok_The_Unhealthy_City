...
Produces BKK_Hospital_Population.html
...
but shows only the "Hospitals (by nearby population) - marker size" filter as the main visible filter.
...
- Builds a marker-size layer where marker radius and color intensity are derived from the hospital's
  "จำนวนประชากรใกล้เคียงที่ต้องรองรับ" column (population).
...
- Output HTML: BKK_Hospital_Population.html
"""
import json
import base64
...
OUT_HTML = "BKK_Hospital_Population.html"
...
# center icon size/anchor (slightly reduced)
ICON_SIZE = (18, 18)
ICON_ANCHOR = (9, 9)
...
# -------------------------
# Marker-size layer (Hospitals by nearby population) - VISIBLE (only main filter)
# -------------------------
...
min_radius = 6
max_radius = 36
max_pop = hospitals[near_pop_col].max() if len(hospitals) > 0 else 0

pop_layer = FeatureGroup(name="Hospitals (by nearby population) - marker size", show=True, control=False).add_to(m)
...
for _, row in hospitals.iterrows():
    try:
        ...
        continue
    val = int(row.get(near_pop_col, 0))
    ...
    # big sized circle (population-based)
    folium.CircleMarker(
        location=[latf, lonf],
        radius=radius,
        color=color_hex,
