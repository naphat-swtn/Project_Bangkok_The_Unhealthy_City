...
Generate Ratchathewi_Hospital_Under_Default.html — focused "under" (public/private)
view for เขตราชเทวี based on BKK_Hospital_Under_Default.py.

Behavior:
- Loads hospitals.csv, communities.csv, districts_bangkok.geojson.
- Computes nearest-assignment (community -> nearest hospital) globally to compute hospital weight
  (weights are computed but not shown in popup per design).
- Computes district metrics globally (so district tooltip numbers are consistent with BKK pages).
- Finds the ราชเทวี polygon and embeds only that district (highlight).
- Shows only hospitals located inside ราชเทวี.
- Shows public/private markers (gov/private). Those FeatureGroups are visible on the map
  but hidden from the LayerControl (control=False) as in BKK variant.
- Hospital popup shows: ชื่อโรงพยาบาล, เขต, เบอร์, เว็บไซต์, ประเภท.
- District tooltip shows เขต / จำนวนโรงพยาบาล / จำนวนชุมชน (global metrics).
- Mobile tap-to-toggle tooltips and click-to-highlight are included.
"""
import json
...
OUT_HTML = "Ratchathewi_Hospital_Under_Default.html"
...
# ensure numeric popup fields exist (kept but not shown)
...
# ---------- Compute nearest assignment (global) to compute hospital weight ----------
...
# ---------- Compute district metrics globally (for tooltip numbers) ----------
...
# ---------- Select hospitals inside the target district (for display only) ----------
hospitals_in = []
for h_idx, hosp in hospitals.iterrows():
    try:
        pt = ShapelyPoint(float(hosp[LON_COL]), float(hosp[LAT_COL]))
...
# ---------- Prepare highlight feature for the district ----------
...
# ---------- Marker layers: public and private (visible but hidden from LayerControl) ----------
...
for h_idx, hosp in hospitals_in:
    try:
        ...
        continue
    hosp_name = hosp.get('โรงพยาบาล') or hosp.get(hosp_name_col) or ''
    hosp_name_esc = esc(hosp_name)
    ...
    # popup content: only name, district, tel, website, and type
    popup_html = f"""
    ...
    if hosp_type == "รัฐ":
        ...
        # unknown type: neutral small marker placed into gov_layer (hidden from control)
...
# ---------- JS: mobile tap-to-toggle tooltips and click-to-highlight for district polygon ----------
