...
Produces BKK_Hospital_Rights_UHC.html

Shows only hospitals that accept 'สิทธิบัตรทอง' (column "สิทธิบัตรทอง" == "YES"),
uses Hospital_UHC.png as marker icon, and popups show:
- ชื่อโรงพยาบาล
- เขต
- เบอร์
- เว็บไซต์
...
Design System follows BKK_Hospital_Default / Under_Private:
...
OUT_HTML = "BKK_Hospital_Rights_UHC.html"

HOSP_ICON_FN = "Hospital.png"       # small icon in popup header (inline if present)
UHC_ICON_FN = "Hospital_UHC.png"    # marker icon for สิทธิบัตรทอง view
...
UHC_ICON_URI = try_file_name(UHC_ICON_FN)
...
# Rights columns (normalize presence)
rights_cols = {
...
for col in rights_cols.values():
    if col not in hospitals.columns:
        hospitals[col] = ""  # treat missing as not accepted

# -------------------------
# Compute nearest assignment (communities -> nearest hospital) to fill weight if needed
# -------------------------
comm_assigned = []
...
# -------------------------
# Compute district metrics and write into geojson properties (so tooltips won't fail)
# -------------------------
district_features = bangkok_geo.get('features', [])
district_name_field = 'amp_th'  # adjust if needed
...
# -------------------------
# UHC (gold card) layer only
# -------------------------
uhc_layer = FeatureGroup(name="Hospitals - สิทธิบัตรทอง", show=True, control=False).add_to(m)

# Filter hospitals that accept gold card (exact match 'YES' assumed)
uhc_hospitals = hospitals[hospitals[rights_cols['gold_card']] == "YES"] if rights_cols['gold_card'] in hospitals.columns else hospitals.iloc[0:0]
...
for _, row in uhc_hospitals.iterrows():
    ...
    # Build rights display Yes/No
    def yesno(col):
        return "Yes" if str(row.get(col, "")).strip().upper() in ("YES", "Y", "TRUE", "1", "รับ") else "No"

    gold_v = yesno(rights_cols['gold_card'])
    sso_v = yesno(rights_cols['sso'])
    gov_v = yesno(rights_cols['gov'])
    ...
    # add marker with UHC icon, fallback to colored circle
    try:
        icon = folium.CustomIcon(UHC_ICON_URI, ICON_SIZE, ICON_ANCHOR)
...
# -------------------------
# CSS (fonts, LayerControl font, tooltip) and keep LayerControl for base maps
# -------------------------
css = """
...
# -------------------------
# Inject JS to support mobile tap-to-toggle tooltips and click-to-highlight
# -------------------------
gj_var = gj.get_name()
map_var = m.get_name()
js_touch_template = """
