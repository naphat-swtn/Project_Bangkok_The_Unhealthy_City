...
Produces BKK_Hospital_Under_Gov.html

This view is identical to BKK_Hospital_Under_Default.html design-wise, but shows only the
"Hospitals - รัฐ (public only)" layer. Private hospital markers are not added to this page.
Popups show: ชื่อโรงพยาบาล, เขต, เบอร์, เว็บไซต์, ประเภท.

Usage:
    python BKK_Hospital_Under_Gov.py
"""
import json
...
OUT_HTML = "BKK_Hospital_Under_Gov.html"
...
# -------------------------
# Compute nearest assignment (communities -> nearest hospital) to keep weight if needed
# -------------------------
comm_assigned = []
...
# -------------------------
# Compute district metrics and write into geojson properties
# -------------------------
...
# -------------------------
# Combined districts layer (fill + bounds) + tooltip
# -------------------------
...
# -------------------------
# Gov layer only (visible); private layer NOT added to this page
# -------------------------
...
for _, row in hospitals.iterrows():
    ...
    if hosp_type != "รัฐ":
        continue  # only add public hospitals

    # popup content: name, district, tel, website, type
    popup_html = f"""
    ...
    # try to use the government icon; fallback to colored circle
    try:
...
# -------------------------
# CSS (fonts, LayerControl fonts and tooltip style)
# -------------------------
css = """
