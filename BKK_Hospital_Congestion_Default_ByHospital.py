...
Produces BKK_Hospital_Congestion_Default_ByHospital.html

Same as previous version but with a slightly smaller Hospital.png center icon size
(to make the center icon less dominant).
"""
import json
import base64
...
OUT_HTML = "BKK_Hospital_Congestion_Default_ByHospital.html"
...
# icon size/anchor for center marker (reduced slightly)
ICON_SIZE = (18, 18)   # reduced from 22x22 to 18x18
ICON_ANCHOR = (9, 9)
...
# Ensure numeric columns for new popup fields exist (coerce to numeric, default 0)
...
# -------------------------
# Marker-size layer (Hospitals by nearby communities) - VISIBLE (only main filter)
# -------------------------
...
for _, row in hospitals.iterrows():
    ...
    # Build popup with required fields
    ...
    <div style="background:#EAF3FF; color:#1A1A1A; font-family: 'Bai Jamjuree', sans-serif; padding:12px; border-radius:8px; border:2px solid #6C7A89; max-width:360px;">
      ...
      <div style="margin-top:8px; font-size:14px;">
        ...
        <div><strong>จำนวนชุมชนใกล้เคียง:</strong> {w}</div>
    ...
    folium.CircleMarker(
        ...
        tooltip=f"{title} — {w} ชุมชน"
    ).add_to(congestion_layer)

    # replace red center marker with Hospital.png icon marker
    try:
        ...
        # fallback to small red dot if icon fails
        folium.CircleMarker(
            location=[latf, lonf],
            radius=7,
    ...
    function scanAndBind(){
      try {
        ...
        map.eachLayer(function(layer){
          try {
            if (layer && layer.eachLayer and layer.options and layer.options.name) {
...
# Only base layers + congestion filter are visible by default; other filters are intentionally omitted.
