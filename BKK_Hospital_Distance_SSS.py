...
Generates:
- BKK_Hospital_Distance_SSS.html

Purpose:
- Similar to BKK_Hospital_Distance_UHC.html but for hospitals that accept
  "สิทธิประกันสังคม" (SSS / social security).
- Shows a "Filter Connections - สิทธิประกันสังคม" visualization:
  gray connection lines from each community to the nearest SSS hospital.
- Community markers use green (stroke + fill).
- Community popup text shows:
    "โรงพยาบาลที่รับสิทธิประกันสังคมใกล้ที่สุด:" (nearest SSS hospital)
  plus distance and population. Popups use Bai Jamjuree font and same sizes as hospital popups.
- Hospital popups show the usual details.
- Districts are embedded (fill + bounds) and moved to back so markers are clickable.
- District tooltip compatibility keys (amp_th / name) are injected.
...
Usage:
  Place this file in the folder with:
    ...
    - House.png
  Run:
    python BKK_Hospital_Distance_SSS.py
  Serve:
    python -m http.server 8000
  Open:
    http://localhost:8000/BKK_Hospital_Distance_SSS.html
"""
import json
...
OUT_HTML = "BKK_Hospital_Distance_SSS.html"
...
# Colors: gray lines, green markers
SSS_LINE_COLOR = "#9E9E9E"      # gray for connection lines
SSS_MARKER_COLOR = "#4CAF50"    # green for marker stroke and fill
...
def detect_sss_column(df_cols):
    # Added exact "สิทธิประกันสังคม" to candidates to match user's CSV
    candidates = [
        'สิทธิประกันสังคม',         # <-- เพิ่มคำนี้
        'รับสิทธิประกันสังคม',
        'SSS',
        'ประกันสังคม',
        'social_security',
...
# ---------- Compute nearest SSS hospital for each community ----------
comm_assigned_sss = []
for c_idx, comm in communities.iterrows():
    ...
    comm_assigned_sss.append((c_idx, nearest_idx, min_d if min_d != float('inf') else None))
...
# ---------- Compute district metrics (using all hospitals for consistency) ----------
...
# ---------- SSS Hospitals layer ----------
sss_layer = FeatureGroup(name="SSS Hospitals (ประกันสังคม)", show=True, control=False).add_to(m)
HOSP_ICON_URI = try_file_name(HOSP_ICON_FN)
for _, row in sss_hospitals.iterrows():
    try:
    ...
    try:
        folium.Marker(location=[latf, lonf],
                      ...
                      tooltip=title_esc).add_to(sss_layer)
    except Exception:
        folium.CircleMarker(location=[latf, lonf], radius=6, color='#c62828', fill=True, fill_color='#c62828',
                            popup=folium.Popup(popup_html, max_width=420), tooltip=title_esc).add_to(sss_layer)

# ---------- Communities (green) and SSS Connections (gray lines) ----------
...
for comm_idx, nearest_idx, dist_m in comm_assigned_sss:
    ...
    # green circle marker (stroke + fill)
...
print("SSS hospitals:", len(sss_hospitals), "SSS connections drawn:", sum(1 for c in comm_assigned_sss if c[1] is not None))