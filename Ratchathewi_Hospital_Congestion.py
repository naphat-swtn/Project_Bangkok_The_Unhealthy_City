...
Generate Ratchathewi_Hospital_Congestion.html — a focused congestion map for
เขตราชเทวี only, with all metrics computed globally (same approach as
BKK_Hospital_Congestion_Default_ByHospital) but the map shows only the ราชเทวี
polygon and hospitals/optional connections inside it. The colormap legend
has been removed per request.

Usage:
  python Ratchathewi_Hospital_Congestion.py
  python -m http.server 8000
  Open http://localhost:8000/Ratchathewi_Hospital_Congestion.html
"""
import json
import base64
...
OUT_HTML = "Ratchathewi_Hospital_Congestion.html"

HOSP_ICON_FN = "Hospital.png"
PUSH_PIN_FN = "RoundPushpin.png"

# center icon slightly reduced (match BKK_ByHospital)
ICON_SIZE = (18, 18)
ICON_ANCHOR = (9, 9)
...
# ---------- Compute assignments and weights USING ALL DATA (global) ----------
# Assign each community to its nearest hospital among ALL hospitals
...
# compute hospital-level metrics globally (num communities, sum population)
...
# global max for normalization (used for marker color/size)
global_max_w = max((v['num_communities'] for v in h_metrics_global.values()), default=1)

# ---------- Compute district metrics globally (so choropleth_norm is global) ----------
district_features = bangkok_geo.get('features', []) or []
district_name_field = detect_name_field(district_features) or 'amp_th'

# build shapes for PIP
district_shapes = []
district_names = []
...
# ---------- For connections to draw: use communities_in assigned (global) to hospitals that are in the district ----------
assigned_lookup = {c_idx: h_idx for c_idx, h_idx, d in comm_assigned_global}

# ---------- Prepare district feature to embed (use global metrics for that district) ----------
...
# ---------- Congestion markers (for hospitals inside target district) but using GLOBAL metrics ----------
small_color_hex = '#ffdede'
large_color_hex = '#b71c1c'
min_radius = 6
max_radius = 36
max_w_global = global_max_w

congestion_layer = folium.FeatureGroup(name="Hospitals (by nearby communities) - marker size", show=True, control=False).add_to(m)
...
for h_idx, hosp in hospitals_in:
    try:
        ...
        continue
    # use global metrics for this hospital
    num_c = h_metrics_global.get(h_idx, {}).get('num_communities', 0)
    sum_pop = h_metrics_global.get(h_idx, {}).get('sum_population', 0)
    normalized = (math.sqrt(num_c) / math.sqrt(max_w_global)) if max_w_global > 0 else 0.0
    ...
    folium.CircleMarker(
        ...
        tooltip=f"{title} — {num_c} ชุมชน"
    ).add_to(congestion_layer)

    # center icon marker (small)
    try:
        icon = folium.CustomIcon(HOSP_ICON_URI, ICON_SIZE, ICON_ANCHOR)
        folium.Marker(location=[latf, lonf], icon=icon, tooltip=f"{title} (center)").add_to(congestion_layer)
    except Exception:
        folium.CircleMarker(location=[latf, lonf], radius=7, color='#d32f2f', fill=True, fill_color='#d32f2f', fill_opacity=1.0, weight=0.6).add_to(congestion_layer)
...
# ---------- NOTE: Colormap legend removed as requested ----------
