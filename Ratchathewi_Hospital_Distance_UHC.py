...
Generate Ratchathewi_Hospital_Distance_UHC.html — focused UHC-distance map for
เขตราชเทวี.

Behavior:
- Detect hospitals that accept สิทธิบัตรทอง (UHC) and compute nearest-UHC assignment
  for every community (global assignment among UHC hospitals).
- Show:
  - UHC hospitals located inside ราชเทวี (marker + popup).
  - Communities that are inside ราชเทวี OR are outside but assigned to a UHC hospital
    that is inside ราชเทวี (yellow markers).
  - Gray connection lines from those communities to their assigned UHC hospital.
  - UHC hospitals outside ราชเทวี that are linked from at least one community inside ราชเทวี
    (shown in a separate "Linked Hospitals (outside ราชเทวี)" layer so you can inspect cross-district links).
- District polygon for ราชเทวี is embedded (tooltip + click-to-highlight) and moved behind markers.
- Output: Ratchathewi_Hospital_Distance_UHC.html
"""
import json
...
OUT_HTML = "Ratchathewi_Hospital_Distance_UHC.html"
...
UHC_MARKER_COLOR = "#FFB300"    # yellow for community markers
UHC_HOSPITAL_COLOR = "#c62828"  # red for UHC hospitals fallback
...
# ---------- Nearest-UHC hospital for each community (global among UHC hospitals) ----------
comm_assigned_uhc = []
for c_idx, comm in communities.iterrows():
    try:
        clat = float(comm[LAT_COL]); clon = float(comm[LON_COL])
    except Exception:
        comm_assigned_uhc.append((c_idx, None, None)); continue
    ...
    comm_assigned_uhc.append((c_idx, nearest, min_d if min_d != float('inf') else None))

# compute UHC hospital weights (num communities assigned)
...
# ---------- UHC hospitals inside Ratchathewi (displayed) ----------
uhc_hospitals_in = []
h_in_set = set()
for h_idx, hosp in uhc_hospitals.iterrows():
    try:
    ...
    try:
        if target_shape.contains(pt):
            uhc_hospitals_in.append((h_idx, hosp))
            h_in_set.add(h_idx)
    except Exception:
        continue
...
# - communities (outside) whose assigned nearest UHC hospital is inside ราชเทวี
comm_to_show = []
for c_idx, h_idx, d in comm_assigned_uhc:
    try:
...
# ---------- Identify linked UHC hospitals outside ราชเทวี that are assigned-to by communities INSIDE ราชเทวี ----------
...
# ---------- UHC hospitals inside ราชเทวี ----------
uhc_in_layer = FeatureGroup(name="UHC Hospitals (in ราชเทวี)", show=True, control=False).add_to(m)
HOSP_ICON_URI = try_file_name(HOSP_ICON_FN)
ICON_SIZE = (18,18); ICON_ANCHOR = (9,9)

for h_idx, hosp in uhc_hospitals_in:
    try:
    ...
    """
    # try icon, fallback to red circle
    try:
        folium.Marker(location=[latf, lonf],
                      icon=folium.CustomIcon(HOSP_ICON_URI, (22,22), (11,11)),
                      popup=folium.Popup(popup_html, max_width=420),
                      tooltip=title_esc).add_to(uhc_in_layer)
    except Exception:
        folium.CircleMarker(location=[latf, lonf], radius=7, color=UHC_HOSPITAL_COLOR, fill=True, fill_color=UHC_HOSPITAL_COLOR,
                            popup=folium.Popup(popup_html, max_width=420), tooltip=title_esc).add_to(uhc_in_layer)

# ---------- Linked UHC hospitals (outside ราชเทวี) - now use Hospital.png icon if available ----------
...
# ---------- Communities (yellow) and connections (gray) ----------
...
for comm_idx, nearest_idx, dist_m in comm_assigned_uhc:
    # show only communities we selected (inside or assigned to in-district UHC)
    if not any(comm_idx == c for c,_,_ in comm_to_show):
        continue
    ...
    # draw connection to assigned UHC hospital (gray) if available
    if nearest_idx is not None and pd.notnull(nearest_idx):
        try:
            hosp = uhc_hospitals.loc[nearest_idx]
            hlat = float(hosp[LAT_COL]); hlon = float(hosp[LON_COL])
...
print("UHC hospitals total:", len(uhc_hospitals), "UHC hospitals in ราชเทวี:", len(uhc_hospitals_in))
print("Linked outside UHC hospitals shown:", len(linked_outside_hospitals_idx))
print("Communities with UHC assignments shown:", len(comm_to_show))