...
Generate Ratchathewi_Hospital_Rights_UHC.html — focused view showing hospitals that
accept สิทธิบัตรทอง (UHC) located inside เขตราชเทวี. Based on BKK_Hospital_Rights_UHC.py.
...
- Shows only hospitals that:
    - accept สิทธิบัตรทอง (UHC) per the detected column, AND
    - are located inside ราชเทวี polygon.
- UHC hospitals are shown with UHC_ICON_FN marker (fallback circle).
- Popups show:
    - ชื่อโรงพยาบาล
    - เขต
    - เบอร์
...
- Mobile touch tap-to-toggle tooltip and click-to-highlight behavior included.
"""
import json
...
OUT_HTML = "Ratchathewi_Hospital_Rights_UHC.html"

HOSP_ICON_FN = "Hospital.png"       # inline small icon in popup header (if present)
UHC_ICON_FN = "Hospital_UHC.png"    # marker icon for UHC hospitals
...
# detect rights columns (include common variants and the user's exact names)
...
# ensure columns exist in dataframe (create empty if missing)
if gold_col is None:
...
# ---------- Determine hospitals in target district that accept UHC ----------
# Use truthy detection for gold_col
hospitals_in = []
for h_idx, hosp in hospitals.iterrows():
    try:
        pt = ShapelyPoint(float(hosp[LON_COL]), float(hosp[LAT_COL]))
...
# filter UHC hospitals (truthy on gold_col)
uhc_hospitals_in = []
for h_idx, hosp in hospitals_in:
    if truthy(hosp.get(gold_col, "")):
...
# ---------- UHC hospitals layer (only in district) ----------
uhc_layer = FeatureGroup(name="Hospitals - สิทธิบัตรทอง (UHC)", show=True, control=False).add_to(m)
...
for h_idx, hosp in uhc_hospitals_in:
    ...
    try:
        ...
        folium.CircleMarker(location=[latf, lonf], radius=6, color='#66bb6a', fill=True, fill_color='#66bb6a',
                            popup=folium.Popup(popup_html, max_width=480), tooltip=title_esc).add_to(uhc_layer)
...
# ---------- JS: bind mobile tap-to-toggle tooltips and click-to-highlight (only district layer) ----------
...
print(f"Hospitals in Ratchathewi accepting UHC: {len(uhc_hospitals_in)}")