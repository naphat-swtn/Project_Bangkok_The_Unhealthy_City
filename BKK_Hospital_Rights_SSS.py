...
Produces BKK_Hospital_Rights_SSS.html

This page follows the same Design System as the other views (Bai Jamjuree font, Thai base tiles,
combined district fill+stroke, district tooltips and mobile tap-to-toggle behavior) but:
- Shows only hospitals that accept "สิทธิประกันสังคม" (Social Security) (column "สิทธิประกันสังคม" == "YES")
- Uses Hospital_SSS.png as the marker icon (if present, otherwise falls back to a colored circle)
- Popup shows:
    - ชื่อโรงพยาบาล
    - เขต
    - เบอร์
...
OUT_HTML = "BKK_Hospital_Rights_SSS.html"

HOSP_ICON_FN = "Hospital.png"       # inline small icon for popup header
SSS_ICON_FN = "Hospital_SSS.png"    # marker icon for สิทธิประกันสังคม view
...
SSS_ICON_URI = try_file_name(SSS_ICON_FN)
...
# -------------------------
# SSO layer only (visible)
# -------------------------
sso_layer = FeatureGroup(name="Hospitals - สิทธิประกันสังคม", show=True, control=False).add_to(m)

# Filter hospitals that accept Social Security (exact match 'YES' assumed)
sso_hospitals = hospitals[hospitals[rights_cols['sso']] == "YES"] if rights_cols['sso'] in hospitals.columns else hospitals.iloc[0:0]

def yesno(val):
    v = str(val or "").strip().upper()
    return "Yes" if v in ("YES", "Y", "TRUE", "1", "รับ") else "No"

for _, row in sso_hospitals.iterrows():
    try:
    ...
    # add marker with SSS icon, fallback to colored circle
    try:
        icon = folium.CustomIcon(SSS_ICON_URI, ICON_SIZE, ICON_ANCHOR)
        folium.Marker(location=[latf, lonf], icon=icon,
                      popup=folium.Popup(popup_html, max_width=420),
                      tooltip=title_esc).add_to(sso_layer)
    except Exception:
        folium.CircleMarker(location=[latf, lonf], radius=6, color='#66bb6a', fill=True, fill_color='#66bb6a',
                            popup=folium.Popup(popup_html, max_width=420), tooltip=title_esc).add_to(sso_layer)
