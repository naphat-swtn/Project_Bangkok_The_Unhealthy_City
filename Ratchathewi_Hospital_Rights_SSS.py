...
Generate Ratchathewi_Hospital_Rights_SSS.html — focused view showing hospitals that
accept สิทธิประกันสังคม (SSS) located inside เขตราชเทวี.
...
- Shows only hospitals that accept สิทธิประกันสังคม (SSS) per detected column and are located in ราชเทวี.
- Marker uses Hospital_SSS.png if present (relative filename) else falls back to a green circle.
- Popups show:
    - ชื่อโรงพยาบาล
    - เขต
    - เบอร์
...
OUT_HTML = "Ratchathewi_Hospital_Rights_SSS.html"

HOSP_ICON_FN = "Hospital.png"       # small icon in popup header (inline if present)
SSS_ICON_FN = "Hospital_SSS.png"    # marker icon for SSS hospitals (relative path preferred)
...
# detect rights columns (include common variants)
...
# filter to SSS-accepting hospitals (truthy on sso_col)
sss_hospitals_in = []
for h_idx, hosp in hospitals_in:
    if truthy(hosp.get(sso_col, "")):
...
# ---------- SSS hospitals layer (only in district) ----------
sss_layer = FeatureGroup(name="Hospitals - สิทธิประกันสังคม (SSS)", show=True, control=False).add_to(m)
...
for h_idx, hosp in sss_hospitals_in:
    ...
    try:
        ...
        folium.CircleMarker(location=[latf, lonf], radius=6, color='#4caf50', fill=True, fill_color='#4caf50',
                            popup=folium.Popup(popup_html, max_width=480), tooltip=title_esc).add_to(sss_layer)
...
print(f"Hospitals in Ratchathewi accepting SSS: {len(sss_hospitals_in)}")